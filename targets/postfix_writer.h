...
namespace mml {
  ...
  class postfix_writer: public basic_ast_visitor {
    ...
    std::stack<int> _retLabels; // where to jump when a return occurs of an exclusive section ends
  ...
  public:
    postfix_writer(std::shared_ptr<cdk::compiler> compiler, cdk::symbol_table<mml::symbol> &symtab,
         ...
         basic_ast_visitor(compiler), _symtab(symtab), _errors(false), _inFunction(false), _inFunctionName(false), _inFunctionArgs(
            false), _inFunctionBody(false), _function(nullptr), _offset(0), _lvalueType(
            cdk::TYPE_VOID), _currentFunctionName(""), _retLabels(), _pf(pf), _lbl(1),
