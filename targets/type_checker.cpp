...
void mml::type_checker::do_size_of_node(mml::size_of_node *const node, int lvl) {
  // EMPTY
}
void mml::type_checker::do_not_node(cdk::not_node *const node, int lvl) {
  ...
  {
    throw std::string("invalid type in logical negation");
  }
}
...
void mml::type_checker::do_declaration_node(mml::declaration_node * const node, int lvl) {
  ...
  if (node->initializer() != nullptr) {
    ...
    std::cout << node->initializer()->type()<< std::endl;
    ...
    if (node->type()->name() == cdk::TYPE_DOUBLE) {
      if (!node->initializer()->is_typed(cdk::TYPE_DOUBLE)) throw std::string("wrong type for initializer (integer expected).");
    }
    if (node->type()->name() == cdk::TYPE_STRING) {
      if (!node->initializer()->is_typed(cdk::TYPE_STRING)) throw std::string("wrong type for initializer (integer expected).");
    }
